...
export async function POST(_req: Request, { params }: Params) {
  ...
  const { data: payment, error: fetchError } = await supabase.from('local_payments').select('*').eq('id', id).single()
  ...
  const { error: enrollError } = await supabase.from('enrollments').upsert({
    ...
    full_name: payment.student_name, phone: payment.student_phone, city: payment.student_city,
