...
export async function POST(request: Request, { params }: Params) {
  ...
  const { title, description, content_type, bunny_video_id, file_url, duration_seconds, is_free, position } = body
  if (!title) return NextResponse.json({ error: 'Title is required' }, { status: 400 })
  const supabase = createAdminClient()
  const { data: course } = await supabase.from('courses').select('id').eq('slug', slug).single()
  if (!course) return NextResponse.json({ error: 'Course not found' }, { status: 404 })
  const { data, error } = await supabase.from('lessons').insert({
    course_id: course.id, title, description: description || null,
    content_type: content_type || 'video',
    bunny_video_id: bunny_video_id || null, file_url: file_url || null,
    video_url: null, duration_seconds: duration_seconds || 0,
    is_free: is_free ?? false, position: position || 1,
  }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function PUT(request: Request, { params }: Params) {
  ...
  const { slug } = await params
  const body = await request.json()
  const { id, title, description, content_type, bunny_video_id, file_url, duration_seconds, is_free } = body
  ...
  const { data, error } = await supabase.from('lessons').update({
    ...
    content_type: content_type || 'video',
    bunny_video_id: bunny_video_id || null, file_url: file_url || null,
    duration_seconds: duration_seconds || 0, is_free: is_free ?? false,
